...
const TeamMemberDetails = (props) => {
  ...
  return (
    ...
    <div className="flex flex-col justify-center w-full px-8 mx-6 my-12 text-center text-gray-100 bg-gray-800 rounded-md md:w-96 lg:w-80 xl:w-64">
      <img
        alt=""
        className="self-center w-24 h-24 -mt-12 bg-gray-500 bg-center bg-cover rounded-full"
        src={image}
      />
      <div className="flex-1 my-4">
        <p className="text-xl font-semibold leading-snug">{name}</p>
        <p className="text-indigo-300">{department}</p>
      </div>
      ...
      <button className="flex items-center justify-center my-4 btn-home">
        <Link href={`/doctorDetailsPage/${_id}`}>
          <a className="tracking-wider text-white uppercase ">See Details</a>
        </Link>
      </button>
    </div>
  );
